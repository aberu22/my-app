...
/* ======================================================
   POST — Create Wan 2.6 Text-to-Video Task
====================================================== */
export async function POST(req) {
  ...
  try {
    ...
    // 🔒 Guard against invalid pricing (future-proof)
    ...
    // ✅ Robust taskId extraction (matches Seedance fix)
    const taskId =
      data?.taskId ||
      data?.task_id ||
      data?.data?.taskId ||
    ...
    console.error("❌ Wan 2.6 POST error:", err);
    ...
    if (creditsDeducted && userId && creditsToCharge > 0) {
      try {
        ...
        console.error("🔥 Wan 2.6 credit refund failed:", refundErr);
      }
    }
